...
public class DatePathHandler
    extends BaseHandler
{
  @Override
  public HttpResponse handle(HttpRequest request) throws JangoException
  {
    return new HttpResponse(request.getNamedPathParameter("year") + " - "
        + request.getNamedPathParameter("month") + " - "
        + request.getNamedPathParameter("day")
        + request.getNamedPathParameter("year") + " - "
        + request.getNamedPathParameter("month") + " - "
        + request.getNamedPathParameter("day")
        + request.getNamedPathParameter("year") + " - "
        + request.getNamedPathParameter("month") + " - "
